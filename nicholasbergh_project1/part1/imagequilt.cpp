...
bool loadImg(char *fName) {
	...
	quilt = CImg<unsigned char>(ss * patchW - sw * (patchW - 1), ss * patchH - sw * (patchH - 1), 1, source.spectrum(), 0);
	...
	for (int y1 = 0; y1 < quilt.height(); y1 += ss - sw) {
		for (int x1 = 0; x1 < quilt.width(); x1 += ss - sw) {
			...
			for (int d = 0; d < numSamples; ++d) {
				...
				if (x1 != 0 && y1 != 0) {
					Patch pUp = patches[x1][y1 - ss + sw];
					Patch pAbove = Patch(pUp.top, pUp.left + sw / 2, pUp.bottom, pUp.right - sw / 2);
					Patch p1 = Patch(p.top, p.left + sw / 2, p.bottom, p.right - sw / 2);
					if (patches[x1 - ss + sw][y1].compRight(p) + pUp.compBottom(p, source) < minDiff) {
						minDiff = patches[x1 - ss + sw][y1].rightDiff + pUp.botDiff;
						patches[x1][y1] = p;
					}
				} else if (x1 != 0) {
					...
					Patch pAbove = Patch(pUp.top, pUp.left + sw / 2, pUp.bottom, pUp.right - sw / 2);
					Patch p1 = Patch(p.top, p.left + sw / 2, p.bottom, p.right - sw / 2);
					if (pUp.compBottom(p, source) < minDiff) {
						minDiff = pUp.botDiff;
						patches[x1][y1] = p;
					}
				} else {
			...
			if (y1 != 0 && x2 == quilt.width()) {
				...
				if (y2 == quilt.height()) {
					quilt.crop(0, 0, quilt.width(), quilt.height() - h);
				}
			}
...
int main(int argc, char *argv[]) {
	if (argc > 6) {
		...
		quilt.save_png(argv[2]);
		quilt.display();
	} else {
		printf("You must parameters like so: /imagequilt [in file] [out file] [square patch size] [# horizontal patches] [# vertical patches] [overlap size] [# source samples per patch]\n");
		return 0;
	}
